...
> **Version**: `v0.1.0`
> **Theme**: 技术原型（CLI 核心）
> **Status**: 🟢 Ready to Start（前置 ADR 已全部 Accept）
> **Created**: 2026-04-25
> **Started**: TBD
> **Target Completion**: TBD（不设硬性工时）
>
...
- [ ] 下文 §三 列出的所有 P0 功能全部实现
- [ ] CI 在 macOS 上跑通 lint / build
- [ ] 端到端验收剧本通过（在干净的 macOS 系统）：
  ```bash
  paam track <some-public-skills-repo>
  paam install <skill-name>
  paam sync
  ls ~/.claude/skills/   # 看到 skill 目录
  paam list              # 看到已安装条目
  ```
- [ ] 关键路径有单元测试（如 git URL 解析、配置读写、tree hash 计算）；覆盖率不强制
- [ ] CHANGELOG.md 记录 v0.1.0 全部条目
- [ ] tag `v0.1.0` 已推送
- [ ] M1-retro.md 已写
...
| 卸载 Skill | `paam uninstall <skill>` | **P1（可推迟）** |
| 启用 / 禁用 | `paam enable/disable <skill>` | **P2（推迟到 M2）** |
| Pin 到特定版本 | `paam pin <skill> <ref>` | **P2（推迟到 M2）** |
| 同名冲突解决 | `--prefer <alias>` | **M1 不处理（要求源唯一）** |
...
| 查看单个 Skill 详情 | `paam info <skill>` | **P1** |
| 搜索 | `paam search <kw>` | **P2（M2）** |
